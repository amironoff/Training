...
namespace BluetoothGattHeartRate
{
    ...
    public class HeartRateService
    {
        ...
        private async void ReadBatteryLevel()
        {
            var characteristics =
                service.GetCharacteristics(new Guid("00000206-37cb-11e3-8682-0002a5d5c51b"));
            if (characteristics.Count > 0)
            {
                var value = await characteristics[0].ReadValueAsync();

                int eventData = DataReader.FromBuffer(value.Value).ReadInt32();
                int decoded = ( (uint)0b11111111000000000000000000000000 & eventData ) >> 24;
            }
        }
